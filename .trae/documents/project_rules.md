...
- **MUST**: 中、大问题修改完成后，必须创建修改历史记录文件
- **MUST**: 修改历史文件存储在 `.trae/documents/开发修改历史/` 目录下
- **MUST**: 文件命名格式：`修改历史_{问题描述}_{YYYY-MM-DD_HH-mm-ss}.md`
- **MUST**: 使用MCP time组件获取时间戳，不得使用系统时间或手动输入
- **MUST**: 文件内容必须包含时间戳信息

### 11.2 修改历史记录内容要求

- **MUST**: 详细记录原始问题描述和现象
- **MUST**: 记录问题分析过程和发现的根本原因
- **MUST**: 详细记录具体修改内容，包括：
  - 修改的工具和平台配置
  - 设置的端口、用户名和密码
  - 修改的字段和属性
  ...
  - 修改的文件和代码位置
- **MUST**: 记录测试验证过程和结果
- **MUST**: 提供AI Agent回溯要点和调试方法论

### 11.3 修改历史文档格式标准

- **MUST**: 使用标准化的文档模板，包含以下章节：
  - 基本信息（时间、问题类型、修改人员、问题编号）
  - 原始问题描述
  - 问题分析过程
  - 具体修改内容
  - 接口一致性调整
  ...
  - 解决方案总结
  - AI Agent回溯要点
- **MUST**: 文档格式必须适合AI Agent阅读和回溯
- **MUST**: 使用清晰的标题层级和代码块格式

### 11.4 修改历史使用规则

- **MUST**: AI Agent在遇到类似问题时，必须先查阅修改历史记录
- **MUST**: 避免重复修改或覆盖已修改的内容
- **MUST**: 基于历史记录进行问题回溯和解决方案参考
- **MUST**: 定期整理和归档修改历史记录
